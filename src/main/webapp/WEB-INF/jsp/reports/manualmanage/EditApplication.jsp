...
<html>
  ...
  <body>
  <c:set var="programWiseDetails" value="${requestScope.applicant.applicationForm.programWiseDetails}"/>
  <div class="bodyDiv">
  <div class="headerDiv">  <%@ include file="/templates/Header.jsp" %>
  </div>
  <%@ include file="/templates/HorizontalNavigation2.jsp" %>
  ...
  <div width="100%" style="text-align: left;margin-left:40px;">
  <label style="font-family:verdana;font-size:12px;color:green;font-weight:bold;">Applicant Name : ${requestScope.applicant.applicantName}</label>
  </div>
<form method="post" action="<%=response.encodeURL(request.getScheme()+"://"+request.getServerName()+":"+request.getServerPort()+request.getContextPath()+"/manualmanage/saveManualAcademicDetails.htm")%>" onsubmit="return validateDetails();">
<table cellspacing="5px" cellpadding="2px" width="95%" border="0">
<input type="hidden" name="applicationNumber" value="${requestScope.applicant.applicationNumber}"/>
<!--  LOOP FOR GROUP WISE COMPONENT STARTS HERE -->
<c:forEach var="groupWiseComponent" items="${requestScope.applicant.applicationForm.groupWiseComponents}">
<tr>
	...
	<td>
		...
		<c:choose>
			<c:when test="${component.boardFlag=='Y'}">
				...
				<adm:select defaultText="-Select Board-" name="boards" class="combobox" size="1" optionList="${requestScope.applicant.applicationForm.boards}" selectedValue="${component.boardValue}" style="width:180px;"/>
			</c:when>
...
<c:if test="${requestScope.applicant.applicationForm.branchAvailability=='Y'}">
<tr>
...
<tr>
	...
	<c:forEach var="programWiseDetail" items="${requestScope.applicant.applicationForm.programWiseDetails}">
...
<c:forEach var="programWiseDetail" items="${requestScope.applicant.applicationForm.programWiseDetails}" varStatus="counter">
<tr>
	<td align="left">
