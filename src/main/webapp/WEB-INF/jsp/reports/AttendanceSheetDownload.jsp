...
<c:set var="ProjectURL" value="${fn:substring(url, 0, fn:length(url) - fn:length(uri))}${req.contextPath}" />
  ...
  <div class="container" style = "width:100%;">
    ...
    <tbody>
 				...
 				<tr>
 				<td align="center"><c:out value='${counter.count}'/>. <input type="checkbox" name="program" id="<c:out value='${counter.count}'/>" value="<c:out value='${program.code}'/>" /></td>
 				<td align="left"><c:out value="${program.description}"/></td>
 				<td align="left"><c:out value="${program.otherProperty1}"/><input type="hidden" name="shift" id="<c:out value='${program.otherProperty1}_PPR'/>" value="<c:out value='${program.otherProperty1}'/>"/></td>
 				<td><input type = "submit" name="btn" value = "M" class="btn btn-primary"/>&nbsp&nbsp<input type = "submit" name="btn" value = "F" class="btn btn-primary"/></td>
 				</tr>
 				</form>
 				</c:forEach>
 	</tbody>
