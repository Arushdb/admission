...
<div class="panelHeader"><div class="panelTitle">Add Programs</div></div>
...
-->	<!--<table style="text-align: right;width:80%;" cellspacing="8">
			<tr>
			<td>
			<form method="post" action="<%=response.encodeURL(request.getScheme()+"://"+request.getServerName()+":"+request.getServerPort()+request.getContextPath()+"/testing/getAppliedPrograms.htm")%>">
    		...
    		<tr>
    		<td width="50%">Password</td><td align="left" width="50%"><input type="password" name="password" id="password" onkeyup="changeStyle(this);" value="${requestScope.password}" autocomplete="off"/></td>
    		</tr>
    		<tr>
    		<td  align="center" colspan="2"><input type="submit" value="Submit" class="customButton"/></td>
    		</tr>
    		</table>-->
    		</form>
			</td>
			...
			<table cellspacing="5px">
    		<form method="post" action="<%=response.encodeURL(request.getScheme()+"://"+request.getServerName()+":"+request.getServerPort()+request.getContextPath()+"/testing/applyTransferApplication.htm")%>">
    		<tr>
