...
<tr>
<td> ${list.application_number}</td> <td>${list.candidateName}</td>  <td>${list.fatherName}</td> <td>${list.gender}</td> <td>${list.category} </td><td><input type="hidden" id="CCAboth" name ="cca_marks" value =""/>Contact toAdministrator</td> <td> <input type="text" id="INTboth" name ="int_marks" value ="" maxlength="6" onkeyup="return onlyNosBOTHmarks(event,this);"/></td> 
</tr>
</table>
</item>
</c:forEach>
